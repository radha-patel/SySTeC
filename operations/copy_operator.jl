...
for n in (10, 100, 1000)
    ...
    eval(@finch_kernel mode=fastfinch function copy_transpose1(c, t) 
        for j = _, i = _ 
            t[j, i] = c[i, j] 
        end
    end)
    ...
    t = Fiber!(Dense(Dense(Element(0.0))), zeros(n, n))
    eval(@finch_kernel mode=fastfinch function copy_transpose2(c, t)
        for j = _, i = _ 
            t[j, i] = c[i, j] 
        end
    end)
    ...
    eval(@finch_kernel mode=fastfinch function copy_transpose3(c, t)
        t .= 0
        for j = _, i = _ 
            t[j, i] = c[i, j] 
        end
    ...
    eval(@finch_kernel mode=fastfinch function copy_transpose4(c, t)
        for j = _, i = _ 
            t[j, i] = c[i, j] 
        end
    end)
