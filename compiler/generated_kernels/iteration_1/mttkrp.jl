...
# eval(@finch_kernel mode=fastfinch function mttkrp_gen(C, A, B)
#     C .= 0
#     for l=_, j=_, k=_, i=_
#         if i <= k && k <= l 
...
# ~4.5ms
eval(@finch_kernel mode=fastfinch function mttkrp_opt1(C, A, B_T)
    C .= 0
    for l=_, k=_, i=_, j=_
        if i <= k && k <= l 
            let ik_eq = (identity(i) == identity(k)), kl_eq = (identity(k) == identity(l))
...
# ~3.5ms
eval(@finch_kernel mode=fastfinch function mttkrp_opt2_1(C, A_nondiag, B_T)
    C .= 0
    for l=_, k=_, i=_, j=_
        if i < k && k < l 
...
# ~240μs
eval(@finch_kernel mode=fastfinch function mttkrp_opt2_2(C, A_diag, B_T)
    C .= 0
    for l=_, k=_, i=_, j=_
...
# ~220μs
eval(@finch_kernel mode=fastfinch function mttkrp_opt2_3(C, A_diag, B_T)
    C .= 0
    for l=_, k=_, i=_, j=_
        if identity(i) <= identity(k) && identity(k) <= identity(l) 
...
# ~220μs
eval(@finch_kernel mode=fastfinch function mttkrp_opt2_4(C, A_diag, B_T)
    C .= 0
    for l=_, k=_, i=_, j=_
        if i <= k && k <= l
            let ik_eq = (identity(i) == identity(k)), kl_eq = (identity(k) == identity(l))
